...
function calcolaMoltiplicatori() {
  ...
  if (isDurataCondizionale(codiceDurataPrincipale)) {
    ...
    if (selectDurataCondizione) {
      ...
      /*
       * Per la durata subordinata NON usiamo la logica "(n - 1)".
       *
       * Motivo:
       * la durata successiva alla condizione è un costo aggiuntivo vero e proprio,
       * quindi anche il primo step deve essere conteggiato.
       *
       * Esempi:
       * - 1 round subordinato  = 1 * costo base
       * - 2 round subordinati = 2 * costo base
       * - 1 intervallo da 10 minuti = 1 * costo base
       */
      if (codeDurataCondizione === "DURC_ROUND_OLTRE") {
        const n = parseInt(document.getElementById("numero-durata-condizione-round").value, 10) || 0;
        costoDurataCondizione = n * valoreBaseDurataCondizione;

      } else if (codeDurataCondizione === "DURC_MIN_10") {
        const n = parseInt(document.getElementById("numero-durata-condizione-minuti10").value, 10) || 0;
        costoDurataCondizione = n * valoreBaseDurataCondizione;
